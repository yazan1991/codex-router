...
const DIRECT_REQUEST_HEADERS = new Set([
  "accept", "openai-beta", "originator", "session_id", "session-id", "thread-id",
  "traceparent", "tracestate", "user-agent", "x-client-request-id",
...
export function directResponsesTarget(provider, pathname, search = "") {
  if (!isDirectResponsesProvider(provider)) throw new Error("Provider does not declare the direct Responses contract.");
  const suffix = /\/responses\/compact$/.test(String(pathname || ""))
    ? "/responses/compact"
    : /\/responses$/.test(String(pathname || "")) ? "/responses" : undefined;
...
export function directResponsesBody(payload, route) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) throw new Error("Direct Responses request body must be an object.");
